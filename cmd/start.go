...
peer discovery. Otherwise, the node can be a peer, in which case it connects to a given bootstrap node when starting up.`,
		PersistentPreRunE: func(c *cobra.Command, args []string) error {
			if found := cfg.CheckPeerCfgExists(ID); found != true {
				return errors.New("no matching peer configuration found for id " + ID)
			}
			return nil
		},
	}

	cmd.PersistentFlags().StringVarP(&ID, "id", "i", config.DefaultPeer.ID, "name of private key to use for peer identity")
